...
# Purpose:
# Created:     2003/06/02
# RCS-ID:      $Id: OMVideoProducerService.py,v 1.3 2005-06-06 17:25:55 turam Exp $
# Copyright:   (c) 2002
...
class VideoProducerService( AGService ):
    ...
    def Start( self ):
        ...
        try:
            ...
            options.append('-F'); options.append(self.framerate.value)
            options.append('-bps'); options.append(self.bandwidth.value)
            options.append('-q'); options.append(self.quality.value)
            options.append('-f'); options.append(self.encoding.value)
            #options.append(' '); options.append(self.standard.value)
            #options.append('-D'); options.append(vicDevice)
            options.append('-D'); options.append('VideoCapture/Test')
            options.append('-p'); options.append('blue_passion')
            #options.append('-p'); options.append(portstr)
            options.append('-name'); options.append("%s(%s)" % (name,self.streamname.value))
            options.append('-email'); options.append(email)
            options.append('-omft'); options.append(self.standard.value)
