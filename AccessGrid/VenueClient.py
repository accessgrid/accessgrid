...
class VenueClient:
    ...
    def SetProfile(self, profile):
        """
        This method sets the profile for the VenueClient, there are currently
        two types of client profiles -- one for users, and one for nodes.
        """
        self.profile = profile
    ...
    def EnterVenue(self, URL):
        ...
        if self.streamDescList != None:
            ...
            for stream in self.streamDescList:
                print "  ", stream.capability.type, stream.capability.role,
                stream.location.host, stream.location.port
...
#        update its host/port (setting the callbacks at instantiation, too)
        ...
        self.coherenceClient.add_callback( Event.ENTER,
                                           self.venueState.AddUser )
        self.coherenceClient.add_callback( Event.EXIT,
                                           self.venueState.RemoveUser )
        self.coherenceClient.add_callback( Event.ADD_DATA,
                                           self.venueState.AddData )
        self.coherenceClient.add_callback( Event.REMOVE_DATA,
                                           self.venueState.RemoveData )
        self.coherenceClient.add_callback( Event.ADD_SERVICE,
                                           self.venueState.AddService )
        self.coherenceClient.add_callback( Event.REMOVE_SERVICE,
                                           self.venueState.RemoveService )
        self.coherenceClient.add_callback( Event.ADD_CONNECTION,
                                           self.venueState.AddConnection )
        self.coherenceClient.add_callback( Event.REMOVE_CONNECTION,
                                           self.venueState.RemoveConnection )
