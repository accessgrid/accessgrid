...
# Purpose:     
# Created:     2003/08/02
# RCS-ID:      $Id: AGServiceManager.py,v 1.61 2004-05-04 20:34:23 turam Exp $
# Copyright:   (c) 2003
...
__revision__ = "$Id: AGServiceManager.py,v 1.61 2004-05-04 20:34:23 turam Exp $"
...
#FIXME - # should error out here later
    ...
    def __DiscoverResources( self ):
        ...
        # If the file does not exist, run SetupVideo.py to create it
        if not os.path.exists(self.resourcesFile):
            cmd = "%s %s" % (os.path.join(AGTkConfig.instance().GetBinDir(),
                                          'SetupVideo.py'),
                             self.resourcesFile)
        ...
        if os.path.exists(self.resourcesFile):
            device = None
            portnames = None
            for line in fileinput.input(files = [self.resourcesFile] ):
