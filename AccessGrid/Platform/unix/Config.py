...
class UnixMimeConfig(Config.MimeConfig):
    """
    ...
    def instance():
        if Config.MimeConfig.theMimeConfigInstance == None:
            MimeConfig()

        return Config.MimeConfig.theMimeConfigInstance
...
if not IsOSX():
    MimeConfig = UnixMimeConfig
else:
    import wx  # sorry, but this is necessary for now
    class MimeConfig(UnixMimeConfig):
        """
        The MimeConfig object encapsulates in single object the management
        of mime types. This provides a cross platform solution so the AGTk
        can leverage legacy configuration and applications for data
        ...
        def GetMimeCommands(self, mimeType = None, ext = None):
            print 'mimetype, ext, = ', mimeType, ext
            if mimeType:
            ...
            cdict = UnixMimeConfig.GetMimeCommands(self, mimeType, ext)
            print 'Unix cdict = ', cdict
        ...
        def GetMimeType(self, extension = None):
            ...
            if not extension:
                ...
                return None
            mimetype = UnixMimeConfig.GetMimeType(self,extension)
            if not mimetype:
