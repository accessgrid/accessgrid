""" 
Comment by Chris Lewis - Victoria, University of Wellington:
The doctest will fail on the minimum Python 2.3 as urandom was introduced
in Python 2.4. As we don't use urandom in actual code, this is considered OK.

Note that use of a non-secret key makes retrieving the data trivial to a
determined attacker. If the attacker has access to the user's preferences file,
then storing a secret key there is pointless as that key has now been
compromised. Unless the security of the key is assured (eg. is typed
interactively by the user) this function should only be considered as
an obscurification of the encrypted information.

IV must also remain the same across sessions, but doesn't have to remain
private.

Code courtesy of:
http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/496737
