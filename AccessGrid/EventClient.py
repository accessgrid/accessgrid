...
#
# Created:     2002/12/12
# RCS-ID:      $Id: EventClient.py,v 1.1 2003-02-03 17:30:17 judson Exp $
# Copyright:   (c) 2002
    ...
    def run(self):
        ...
        while self.sock != None and self.running:
            try:
                ...
                if size > 0:
                    # Then read the event
                    pdata = self.sock.read(size)
                    
                    # Unpack the data
                    event = pickle.loads(pdata)
                    if self.running:
                        # Send it on its way
                        if self.callbacks.has_key(event.eventType):
                            self.callbacks[event.eventType](event.data)
                        else:
