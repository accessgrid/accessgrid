...
class UMTPAgent:
	...
	self.SetExitCB(self.SwithToMulticast)

    def __init__(self, log):
        print 'UMTPAgent for test'
        self.log    = log
	...
	self.apm.SetOptions(self.umtp_server)
	umtp_agent = self.GetAgent()
	umtp_agent = (umtp_agent[0], int(umtp_server[1]) + 1) # to match port number
	self.SetAgent(umtp_agent)

    def GetServer(self):
        return self.umtp_server
	
    def StopAgent(self, message):
        self.log.debug("UMTP: stop agent")

    """
     Switch userinterface after using UMTP - default is multicast
    """
    def SwithToMulticast(self, transport = 'multicast'):
        if transport == 'multicast':
            self.controller.UseMulticastCB()
	"""
	 Calls SetTransport in VenueClientUI.py
	"""
	self.__venueClient.SetTransport(transport)

    def ExecuteUMTP(self, old_tx, new_tx):
        """
         Start and Stop UMTP
        """
        log.debug("*** ExecuteUMTP *** (%s, %s)", old_tx, new_tx)
        if old_tx != 'umtp' and new_tx == 'umtp':
            currentUMTP = self.__venueClient.GetCurrentBridge()
            umtp_server = ( currentUMTP.host, currentUMTP.portMin )
            print '----- Forcing umtp host/port to milton.mcs.anl.gov/52000'
            umtp_server = ( 'milton.mcs.anl.gov', 52000)
            
            self.SetStreamList(self.__venueClient.GetStreams())
            self.SetServer(umtp_server)
            self.Start()
        elif old_tx == 'umtp' and new_tx != 'umtp':
            self.Stop(new_tx) # let agent know new tranport method

    def GetTransport(self):
        """
	 Get current transport
	"""
	currentTx = self.__venueClient.GetTransport()
	currentBridge = self.__venueClient.GetCurrentBridge()

	if currentTx == "multicast":
	    return currentTx
	elif currentTx == "unicast":
	    if currentBridge:
	        if currentBridge.serverType == QUICKBRIDGE_TYPE:
		    return currentTx
		elif currentBridge.serverType == UMTP_TYPE:
		    return "umtp"
	    else:
	        return currentTx
	    
    def IsNewTransport(self, old_tx, new_tx):
        """
         check transport value, umtp is same as multicast
        """
	self.ExecuteUMTP(old_tx, new_tx)

        if old_tx == 'umtp': old_tx = 'multicast'
	if new_tx == 'umtp': new_tx = 'multicast'
    
	if old_tx == new_tx: return 0
	else:                return 1 
...
class AgentCommunication:
	...
	for s in self.streamList:
	    for netLoc in s.networkLocations:
		    ...
		    self.log.debug("UMTP: join %s %d", netLoc.host, netLoc.port)
...
class AgentProcessManager:
    ...
    def Start( self ):
        ...
        self.executable = '/Users/turam/dev/umtp/umtp'
        print 'starting umtp ', self.executable, self.options
...
class GroupManagementProtocol:
    ...
    def Assemble(self, command, g_addr, g_port):
        print 'Assemble: ', socket.htonl(g_addr)
...
class AgentError:
	...
	"""
        if not self.IsMyAgent(new_agent_ip):
            ...
            print 'FindNeighbor:', new_agent_ip
    ...
    def Message(self, message, style = 0):
        print '** ERROR : ', message
